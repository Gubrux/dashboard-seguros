            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  :key="column.key"
                  @click="sortingArr(column.key)"
                  style="cursor: pointer"
                  :class="{ 'text-end': column.isNumeric }"
                >
                  {{ column.label }}
                </th>
              </tr>
            ...
            <tbody>
              <tr v-for="(item, index) in paginatedData" :key="index">
                ...
                <td
                  ...
                  :class="{ 'text-end': column.isNumeric }"
                >
                  <span v-if="column.key === 'ejercicio'">{{
                    item.ejercicio
                  }}</span>
                  <span v-else-if="column.key === 'proporcion'">{{
                    moneyFormatter(item.proporcion)
                  }}</span>
                  <span v-else>{{ moneyFormatter(item[column.key]) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
